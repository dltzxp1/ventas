...
if ($fo_id == '0') {
    ...
    if (count($arrFoto) > 0) {
        ...
        echo "<thead>";
        echo "<th align='center'><input type='checkbox' id='chkPadreRol' onclick='selecAllChkBox(\"chkPadreRol\",\"chkHijoRol\")' /></th><th>Nombre</th><th>Fecha</th> <th>Persona</th> <th>  Acciones</th>";
        echo "</thead>";
        echo "<tbody>";
        for ($r = 0; $r < count($arrFoto); $r++) {
            echo "<tr>";
            echo "<td><input name='chkHijoRol' type='checkbox' id='" . $arrFoto[$r]->ca_id . "' onclick='deselecChkPadre(\"chkPadreRol\");' /></td>";
            ...
            for ($i = 0; $i < count($arrPersona); $i++) {
                if ($arrPersona[$i]->pe_id == $arrFoto[$r]->pe_id) {
                    //echo $arrPersona[$i]->cat_nombre;
                    if (strlen($arrPersona[$i]->pe_nombre) >= 15) {
                        echo substr($arrPersona[$i]->pe_nombre, 0, 15) . '.';
                    } else {
                        echo $arrPersona[$i]->pe_nombre;
                    }
                }
            }
